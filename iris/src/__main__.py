...
from iris.src.configuration.config_parser import validate_config_ini
from iris.src.control_center.commands import delete
from iris.src.core.api.keyboard.key import KeyModifier
from iris.src.core.api.keyboard.keyboard import type
from iris.src.core.api.keyboard.keyboard_util import check_keyboard_state
from iris.src.core.api.os_helpers import OSHelper
from iris.src.core.api.settings import Settings
from iris.src.core.util.cleanup import init, CleanUp
from iris.src.core.util.target_loader import collect_tests
from iris.src.core.util.arg_parser import get_core_args, set_core_arg
from iris.src.core.util.json_utils import create_target_json
from iris.src.core.util.local_web_server import LocalWebServer
from iris.src.core.util.logger_manager import initialize_logger
from iris.src.core.util.path_manager import PathManager
from iris.src.core.util.system import (
    check_7zip,
...
def get_target(target_name):
    ...
    print("\n\n\nget_target\n{}\n\n\n".format(target_name))
    try:
        my_module = importlib.import_module("iris.targets.%s.main" % target_name)
        try:
...
def init_control_center():
    copy_tree(
        os.path.join(PathManager.get_module_dir(), "src", "control_center", "assets"),
...
def migrate_data():
    iris_1_install = os.path.join(
        os.path.expanduser("~"), ".iris", "data", "all_args.json"
    )
